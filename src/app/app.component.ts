...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'arkadiko'): Promise<any> {
  ...
  reloadArkadikoIframe() {
    const iframe = document.getElementById('arkadikoIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectArkadikoInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to Arkadiko:', accounts);
                  })
                  .catch(err => console.error('Error connecting to Arkadiko:', err));
              } else {
                alert('MetaMask not found');
              }
            }
