...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'ensdomains'): Promise<any> {
  ...
  reloadEnsdomainsIframe() {
    const iframe = document.getElementById('ensdomainsIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectEnsdomainsInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to ENS Domains:', accounts);
                  })
                  .catch(err => console.error('Error connecting to ENS Domains:', err));
              } else {
                alert('MetaMask not found');
              }
            }
