...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'tokenfi'): Promise<any> {
  ...
  reloadTokenfiIframe() {
    const iframe = document.getElementById('tokenfiIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectTokenfiInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to Tokenfi:', accounts);
                  })
                  .catch(err => console.error('Error connecting to Tokenfi:', err));
              } else {
                alert('MetaMask not found');
              }
            }
