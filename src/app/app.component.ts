...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'apporderly'): Promise<any> {
  ...
  reloadApporderlyIframe() {
    const iframe = document.getElementById('apporderlyIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectApporderlyInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to AppOrderly:', accounts);
                  })
                  .catch(err => console.error('Error connecting to AppOrderly:', err));
              } else {
                alert('MetaMask not found');
              }
            }
