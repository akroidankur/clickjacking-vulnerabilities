...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'etherfi'): Promise<any> {
  ...
  reloadEtherfiIframe() {
    const iframe = document.getElementById('etherfiIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectEtherfiInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to EtherFi:', accounts);
                  })
                  .catch(err => console.error('Error connecting to EtherFi:', err));
              } else {
                alert('MetaMask not found');
              }
            }
