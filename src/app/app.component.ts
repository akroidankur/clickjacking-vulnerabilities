...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'renzo'): Promise<any> {
  ...
  reloadRenzoIframe() {
    const iframe = document.getElementById('renzoIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectRenzoInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to Renzo:', accounts);
                  })
                  .catch(err => console.error('Error connecting to Renzo:', err));
              } else {
                alert('MetaMask not found');
              }
            }
