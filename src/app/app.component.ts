...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'uniswap'): Promise<any> {
  ...
  reloadUniswapIframe() {
    const iframe = document.getElementById('uniswapIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectUniswapInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to Uniswap:', accounts);
                  })
                  .catch(err => console.error('Error connecting to Uniswap:', err));
              } else {
                alert('MetaMask not found');
              }
            }
