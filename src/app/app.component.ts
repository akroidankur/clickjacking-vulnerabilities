...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'appetherfi'): Promise<any> {
  ...
  reloadAppetherfiIframe() {
    const iframe = document.getElementById('appetherfiIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectAppetherfiInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to AppEtherFi:', accounts);
                  })
                  .catch(err => console.error('Error connecting to AppEtherFi:', err));
              } else {
                alert('MetaMask not found');
              }
            }
