...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'pancakeswap'): Promise<any> {
  ...
  reloadPancakeswapIframe() {
    const iframe = document.getElementById('pancakeswapIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectPancakeswapInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to Pancakeswap:', accounts);
                  })
                  .catch(err => console.error('Error connecting to Pancakeswap:', err));
              } else {
                alert('MetaMask not found');
              }
            }
