...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'term'): Promise<any> {
  ...
  reloadTermIframe() {
    const iframe = document.getElementById('termIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectTermInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to Term:', accounts);
                  })
                  .catch(err => console.error('Error connecting to Term:', err));
              } else {
                alert('MetaMask not found');
              }
            }
