...
export class AppComponent implements AfterViewInit {
  ...
  async connectMetamask(target: 'malicious' | 'mintharvest'): Promise<any> {
  ...
  reloadMintharvestIframe() {
    const iframe = document.getElementById('mintharvestIframe') as HTMLIFrameElement;
    if (iframe) {
      iframe.onload = () => {
        this.connectMintharvestInIframe();
      };
            ...
            if (event.data === 'connectWallet') {
              if (window.ethereum) {
                window.ethereum.request({ method: 'eth_requestAccounts' })
                  .then(accounts => {
                    console.log('Connected to MintHarvest:', accounts);
                  })
                  .catch(err => console.error('Error connecting to MintHarvest:', err));
              } else {
                alert('MetaMask not found');
              }
            }
